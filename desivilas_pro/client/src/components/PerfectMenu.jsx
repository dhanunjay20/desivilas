...
const MenuCard = ({ name, image, colIndex, onNavigate }) => {
  ...
  return (
    ...
    >
      ...
      <div className="pt-8 pb-10 text-center">
        <h4 className="text-2xl md:text-3xl font-extrabold text-zinc-900">{name}</h4>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 text-center">
        ...
        <motion.div variants={gridStagger} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.25 }} className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {menuCategories.map((c, idx) => (
            <MenuCard key={c.name} name={c.name} image={c.image} colIndex={idx % 3} onNavigate={goToMenu} />
          ))}
        </motion.div>
        ...
        <motion.div variants={sectionFade} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.5 }} className="mt-20 flex justify-center">
          <Buttons label="Explore Menu" onPrimaryClick={goToMenu} onArrowClick={goToMenu} />
        </motion.div>
      </div>
    </section>
  );
